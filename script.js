...
function addToCart(name, price) {
  // 'this' refers to the button that was clicked, thanks to the inline onclick attribute.
  const button = this;
  const product = button.closest('.product');
  const img = product ? product.querySelector('img') : null;
  
  if (img) {
    // Clone the product image to animate it
    let flyingImg = img.cloneNode(true);
    flyingImg.style.position = "absolute";
    flyingImg.style.width = "50px"; // Adjust the size if needed
    flyingImg.style.transition = "transform 0.6s ease-in-out, opacity 0.6s ease-in-out";
    ...
    // Define a fixed destination (top right corner of the viewport)
    let destinationX = window.innerWidth - 70; // Adjust for desired offset
    let destinationY = 20; // Adjust for desired offset
    
    // Start the animation
    setTimeout(() => {
      flyingImg.style.transform = `translate(${destinationX - rect.left}px, ${destinationY - rect.top}px) scale(0.5)`;
      flyingImg.style.opacity = "0";
    }, 100);
    
    // Remove the cloned image and complete add-to-cart action after animation
    setTimeout(() => {
      flyingImg.remove();
      
      // Update the cart in localStorage and redirect
      let cart = JSON.parse(localStorage.getItem("cart")) || [];
    ...
    // If no image is found, simply add the product and redirect immediately
    let cart = JSON.parse(localStorage.getItem("cart")) || [];
    cart.push({ name, price });
    localStorage.setItem("cart", JSON.stringify(cart));
    window.location.href = "payment.html";
